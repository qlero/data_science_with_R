...
3. We simulate different Markov Chains (e.g. simulate a Bernoulli sequence with one or more different methods (Bernoulli function or uniform discrete on 0, 1), i.e. $\mathcal{B}(p)\sim U[0,1]$, $Y=\mathbb{1}_{[0,p]}(U)$)
    
## Implementation
    
### Steps 1 & 2 - Declaring a parameter space/initial state

**GOAL** -- We decide to initialize our Markov Chain to $0$, and generate the jump probabilities $\mathbb{P}_{0\rightarrow1}$ and $\mathbb{P}_{1\rightarrow0}$ as uniform random variables on the $[0,1]$ support, rounded to 2 decimals. 

We also specify other parameters:

- $n$, the number of Markov Chains generated
- $t$ the length of each chain

**METHOD** -- To generate a set of parameters $(\text{initialization value}, n, t, \mathbb{P}_{0\rightarrow1}, \mathbb{P}_{1\rightarrow0})$, we declare a function `generate_params`.
...
**RESULT/EXAMPLE** -- We display an example of generated Markov chain parameter set:
...
**RESULTS** -- With the two approaches declared above, we can generate Markov chains. We now want to see which of the two is the fastest. This can be achieved with ``system.time()`` base R function. To do so, we arbitrarily select a large $n$ and $t$ as we generate a set of parameters.
...
We see that the second function is faster by c. one order of magnitude. this can be explained by the absence of branching condition (i.e. `if` blocks) in the second function's body. Branching conditions are time consuming operations for a CPU as it requires the use of [branch predictors](https://en.wikipedia.org/wiki/Branch_predictor) that can, and in this case do, waste time. As such, we will use the second function to generate Markov chains.

To generate Markov Chains, we arbitrarily set $t=25$ for data visualization purposes (we want to plot the data). We could set $n=4$ to get a single $4\times25$ matrix of Markov chains, but we would like to display different examples of generated Markov chains, as such, we will simply run the simulation function four times with different parameter sets.
...
**RESULTS** -- We now generate the parameter set as well as the corresponding Markov chain $(Y_n)$. To do so, we can reuse our previously declared simulation function. We arbitrarily select our parameters $\alpha=5$, $\beta=10$, $\delta=0.01$, $n=1000$ and $t=5000$ (the rationale is that it is a high enough amount of simulations while also being low enough that the R Markdown does not take too much time to knit). 
...
Now that have generated 1000 $(Y_n)$, we want to have a preliminary view of their resulting change of states. To do so, we pick the first four simulations for visualization (and only visualize the first 500 timesteps):
...
**COMMENTS** -- Of note, when it is said that $\delta$ is a measure accuracy parameter, it implies that $\delta$ represents the frequency of acquisition of the state of an underlying Markov process. I.e. the smaller $\delta$ is, the more timesteps are acquired in a given timeframe (e.g. a second). 
...
**METHOD** -- To do so, we retrieve the index of the first time $(Y_n)$ jumps to the value $1$ at different $\delta$ (via our previously declared simulation process) and evaluate the overall distribution of $tau_1$. 

Of note, we decrement all retrieved index values by 1 as R is 1-indexed and we know that an exponential distribution has $\mathbb{R}_+$ for support (we want to start at $t=0$ where the initial state is 0).
...
We now have all the indexes of the first state jump to 1 of our previously declared $(Y_n)$. We can evaluate the distribution of $\tau^\delta_1$ where $\delta=0.01$.
...
**COMMENTS** -- As noted in comment in the body of the function `fetch_indexes`, The multiplication by $\delta$ of the index of the first jump to state 1 comes from that we need to rescale the jump timing as $\delta$ varies. Indeed, and as we previously noted, $\delta$ is a frequency acquisition parameter. Rescaling allows us to compare distributions of $\tau_1^\delta$ with different $\delta$ parameters as we're going to see next.

This can be repeated (for the functions available above) for any $\delta\le0.01$. The functions breaks if $\delta$ is higher than this threshold due to the function stack relying on the base R function `rbinom`). For the evaluation of various density given different $\delta$ parameters, refer to the next step where they are generated and displayed.
 
### Step 3 - Visualizing the empirical distribution of $\tau_1^\delta$

**GOAL** -- We are now interested in varying the value of the parameter $\delta$ and observe the impact on the resulting distribution of $\tau_1^\delta$ compared to that of the Exponential distribution with parameter $\alpha$ shared with the underlying Markov process yielding the variable $\tau_1^\delta$. 

**METHOD** -- To do so, we declare our parameter space, along with a sequence of values for $\delta$ to be mapped over.
...
**RESULTS** -- As can be seen below, we see that, as the parameter delta is halved at each of the six simulations (starting from 0.1), it seems that the empirical distribution of $\tau^\delta_1$ converges towards that of the exponential distribution that shares the same parameter $\alpha$ (here  5) of the underlying $(Y_n)$ simulation that led to the computation of $\tau^\delta_1$. 

As such, it seems to confirm our previous observation that $\delta$ represents the frequency of acquisition of the state of an underlying Markov process and that the smaller $\delta$ is, the finer our simulation of the variable $\tau^\delta_1$. 
...
**COMMENTS** -- Though we do not present a mathematical proof that $\tau^\delta_1$ and the Exponential distribution with parameter $\alpha$ converges in distribution for an underlying continuous time Markov process, the empirical evidence seems to validate this state.
