...
&=\frac{\mathbb{P}(N^a_T=k,N^b_T=n-k)}{\mathbb{P}(N^b_T=n-k)}\\
&=\frac{\mathbb{P}(N^a_T=k).\mathbb{P}(N^b_T=n-k)}{\mathbb{P}(N^b_T=n-k)}\\
&=\mathbb{P}(N^a_T=k)\\
\end{align}

As such, we find:

The the conditional probability $N^a_T=k|N_T=n$, by independence between $N^a_T$ and $N^b_T$ also follows a Poisson distribution with parameter/rate $\Lambda_a(0, T)$:

$$\mathbb{P}(N^a_T=k|N_T=n)=\mathbb{P}(N^a_T=k)$$
