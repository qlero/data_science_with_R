...
**E** -- Let's compare $IC_1$ and $IC_2$ for various $N$ sizes. We explain why $IC_2$ cannot converge to $IC_1$ when $N$ goes towards infinity, even if it is a pretty good approximation.

**F** -- We apply the previous bootstrap heuristic on the data from the package STAR to show that there is a clear difference in neuron 1 (experiment `citronellal`) between the period "before the puff", "during the puff", and "after the puff". We will change the level of the confidence interval for Bonferroni correction.
...
In general, we find that the hold-out method tends to generally provide an equivalent or better estimation than v-fold cross-validation given the case used above (i.e. with this specific selection of $h$ bandwidth and $u$ range. Once, it happened that the hold-out and v-fold cross-validation methods found these approximations (*not during the same simulation*), which are very close to the actual sample density:
...
We plot the extracted datasets against each other:
...
We clearly see that there is a discrepancy between the distributions. We now have to show it statistically. We first start by declaring our functions:
...
We now can compute the 95% confidence interval with Bonferroni correction. Here the number of tests of level $\alpha=0.05$ is $3$ since we do one confidence interval per dataset (before, during, after). For the bootstrapping, we reuse the previous number of simulations $N=10000$.
...
As such, we evidence that there is a clear distinction between the three periods in terms of confidence interval of the underlying parameter $\theta$. 

# 3 - Non-Parametric Boostrap

## Overview 

Reusing the previous data of the STAR package, we will perform non-parametric bootstrap (with firing rates instead of ISI). Let's compute to compute non parametric bootstrap confidence intervals on the firing rates. A firing rate is defined by the average number of spikes per second.
...
To compute an estimation of the firing rate, we decide to take the average firing rate per second over each of the three periods outlined in the previous section. We first declare the corresponding function
...
We now extract the data and compute the corresponding firing rates.
...
With the firing rate computes, we can visualize them for each simulation.
...
We build a non-parametric bootstrap function for the mean confidence interval:
...
We generate a normal distribution with 10 elements with mean $\mu=0$ and standard deviation $\sigma=1$, and compute its $95\%$ bootstrapped confidence interval for the mean with $N_{simulation}=10000$ as an example:
...
We see that the real mean $\mu=0$ falls into the bootstrapped confidence interval for the mean.

## Step C

We compute a $95\%$ bootstrapped confidence interval for the mean of each period, with $N_{simulation}=10000$. 
...
We find that the $95\%$ confidence intervals for each period tend to not overlap with each other (Note: in few cases the confidence intervals for the means before and after the puff overlap at their respective upper and lower bounds), indicating that there is a difference between the three periods in terms of firing rates.
...
**A** -- Let's plot for some neurons the superposition of the 4 forms that are corresponding to the action potential as seen by each of the electrodes. We will see that the heights of the signal are very differents. This comes from the proximity of the electrodes. We will also see and that the highest signal is not always recorded by the same electrode.

**B** -- Let's focus on the best shape, i.e., the shape with the highest peak on all electrodes. On this curve, we extract two parameters that represents the action potential: the duration of the peak above half its height, the delay between the time of the valley after the peak and the time of the peak. Let's extract these two values for each of the neuron.

**C** -- Before doing hierarchical clustering, let's center and renormalize both quantities (as when doing PCA) and put the result into a matrix $x$ with 2 columns and 86 rows. Then, let's use `plot(hclust(dist(x),method="ward.D2"))` to see the dendogram. Find the largest distance between clusters and cut the tree there. Conclude that clearly we have two clusters.
...
Once the data is imported, we declare a plotting function to plot selections of neurons from the available data (acquired via each electrode).
...
We select four random neurons and plots their action potentials with the previously declared functions.
...
To see which electrode recorded the highest signal for each neuron, we extract the whole data into a dataframe.
...
We see that in general, the electrodes 2 and 3 recorded the most times the highest signal, while the electrode 4 recorded the highest signal the lowest amount of times among the 86 neurons.

## Step B

We create a function to output the vector $(\text{peak_to_valley_length}, \text{peak_duration})$ from the best available action potential shape per neuron.
...
We display the first five results:
...
We start by reformatting our data into a $86\times2$ matrix and standardizing its content:
...
We now compute the dendrogram of the centered and normalized set of data:
...
```
We want to find the largest distance between clusters and cut the tree there. Iterating over the sequence $\{0.25, 0.5,...,2.75\}$ and then trying with an infinite height, we visualize the resulting dendrogram clustering.
We find that the larest distance between clusters is achieved once height reaches a value between $1.25$ and $1.5$.

```{r cluster_visualization, out.width="60%"}
...
As such, we can confidently say that we have two distinct clusters of action potentials such that:
...
We plot the neurons in the plane of the two previously declared variables with a different color code for each cluster as estimated by a k-means method. We have identified "fast spiking interneurons" (i.e. neurons with small durations for each delays) and the "medium spiny neurons".
